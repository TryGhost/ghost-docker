...
for (const mode of ['local', 'production', 'activitypub']) {
  test(
    `checkpoint and isolated restore of a real Ghost site (${mode})`,
    { skip: enabled ? false : 'set GD_TEST_RECOVERY=1 with Docker', timeout: 1_800_000 },
    async () => {
      ...
      const result = (r) => {
        assert.equal(r.status, 0, r.output || `${r.stdout}\n${r.stderr}`);
        return r;
      };
      const envGet = (site, key) => shOk(`env_get ${q(`${site}/.env`)} ${q(key)}`).trim();
      const manager = execFileSync(
        'docker',
        ['build', '-q', '-f', `${REPO_DIR}/manager/Dockerfile`, REPO_DIR],
        { encoding: 'utf8' },
      ).trim();
      const recover = (site, args) =>
        run(`${site}/scripts/recovery.sh`, args, {
          cwd: site,
          env: { GD_MANAGER_IMAGE: manager },
          timeout: 900_000,
        });
      const interrupt = async (target, argv, atPhase) => {
        const child = spawn(`${target}/scripts/recovery.sh`, argv, {
          cwd: target,
        ...
        while (child.exitCode === null && Date.now() < deadline) {
          try {
            ...
            if (record.phase === atPhase) {
              const name = fs
                .readFileSync(`${target}/.ghost-operation-lock/manager`, 'utf8')
                .trim();
      ...
      try {
        ...
        const modeArgs =
          mode === 'production'
            ? ['--domain', 's4-recovery.test']
            : ['--local', ...(mode === 'activitypub' ? ['--with', 'activitypub'] : [])];
        result(
          run(
            `${source}/install.sh`,
            [...modeArgs, '--no-prompt', '--version', '6.62.0-next-alpine'],
            { cwd: source, timeout: 900_000 },
          ),
        );
        // A database value and hidden asset make this more than a readiness test.
        const sql = (site, query) =>
          compose(
            site,
            [
              'exec',
            ...
            { env: { MYSQL_PWD: envGet(site, 'DATABASE_ROOT_PASSWORD') } },
          );
        if (mode === 'activitypub') {
          result(
            sql(
              source,
              "CREATE TABLE activitypub.s4_recovery_fixture (value VARCHAR(64)); INSERT INTO activitypub.s4_recovery_fixture VALUES ('federation state')",
            ),
          );
        }
        ...
        result(
          compose(source, [
            ...
            'node',
            '-e',
            'require("fs").writeFileSync(process.env.paths__contentPath + "/images/.recovery-drill", "hidden asset survives a checkpoint\\n")',
          ]),
        );
        result(
          run(
            `${source}/scripts/config.sh`,
            ['set', `${source}/ghost.env`, 'mail__from', 'Recovery <hello@example.com>'],
            { cwd: source },
          ),
        );
        result(
          run(
            `${source}/scripts/config.sh`,
            ['set', `${source}/ghost.env`, 's4__literal', literal],
            { cwd: source },
          ),
        );
        ...
        const checkpoint = path.join(
          source,
          '.ghost-backups',
          fs.readdirSync(path.join(source, '.ghost-backups')).find((n) => !n.startsWith('.')),
        );
        ...
        const expectedTheme = result(
          sql(source, "SELECT value FROM settings WHERE `key`='active_theme'"),
        ).stdout;
        if (mode !== 'local') {
          result(compose(source, ['down']));
        }
        const restoreArgs = mode === 'local' ? ['--local', '--port', '24879'] : [];
        await interrupt(
          dest,
          ['restore', checkpoint, '--project', `gd-restore-${process.pid}`, ...restoreArgs],
          'verifying',
        );
        assert.equal(
          compose(dest, ['ps', '--status', 'running', '--services']).stdout.trim(),
          'db',
        );
        ...
        assert.equal(
          fs.readFileSync(`${dest}/data/ghost/images/.recovery-drill`, 'utf8'),
          fs.readFileSync(asset, 'utf8'),
        );
        assert.equal(
          result(sql(dest, "SELECT value FROM settings WHERE `key`='title'")).stdout.trim(),
          'Recovery drill title',
        );
        assert.equal(
          result(sql(dest, "SELECT value FROM settings WHERE `key`='active_theme'")).stdout,
          expectedTheme,
        );
        if (mode === 'activitypub') {
          assert.equal(
            result(sql(dest, 'SELECT value FROM activitypub.s4_recovery_fixture')).stdout.trim(),
            'federation state',
          );
        }
        ...
        const actualLiteral = result(
          compose(dest, [
            ...
            'node',
            '-e',
            'console.log(JSON.stringify(process.env.s4__literal))',
          ]),
        ).stdout.trim();
        assert.equal(JSON.parse(actualLiteral), literal);
        assert.equal(
          envGet(dest, 'RESTART_POLICY'),
          mode === 'production' ? 'unless-stopped' : 'no',
        );
        if (mode !== 'production') {
          const response = await fetch(
            `http://127.0.0.1:${envGet(dest, 'GHOST_PORT')}/ghost/api/admin/site/`,
          );
          ...
          const response = run('curl', [
            ...
            '-ksS',
            '--resolve',
            's4-recovery.test:443:127.0.0.1',
            '-o',
            '/dev/null',
            '-w',
            '%{http_code}',
            'https://s4-recovery.test/ghost/api/admin/site/',
          ]);
          assert.equal(result(response).stdout, '200');
        }
      } finally {
        for (const site of [source, dest]) {
        ...
        if (process.env.GD_KEEP_RECOVERY_TEST !== '1') {
          execFileSync(
            ...
            [
              ...
              'sh',
              '--mount',
              `type=bind,source=${dir},target=/fixture`,
              manager,
              '-c',
              'chown -R "$1" /fixture; chmod -R u+rwX /fixture',
              '--',
          ...
          console.log(`Retained fixture: ${dir}`);
        }
      }
    },
  );
