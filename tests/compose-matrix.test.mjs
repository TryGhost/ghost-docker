...
const MATRIX = [
  ...
  {
    profiles: 'local,activitypub',
    mode: 'local',
    services: ['activitypub', 'activitypub-migrate', 'db', 'ghost'],
  },
  {
    profiles: 'local,analytics,activitypub',
    mode: 'local',
    services: [
      'activitypub',
      'activitypub-migrate',
  ...
  {
    profiles: 'production,activitypub',
    mode: 'production',
    services: ['activitypub', 'activitypub-migrate', 'caddy', 'db', 'ghost'],
  },
  {
    profiles: 'production,analytics,activitypub',
    mode: 'production',
    services: [
      'activitypub',
      'activitypub-migrate',
        ...
        for (const entry of MATRIX) {
          describe(entry.profiles, () => {
            ...
            test('services have unique network aliases', () => {
              const aliases = (name) =>
                config.services[name]?.networks?.ghost_network?.aliases ?? [];
              ...
              if (entry.services.includes('traffic-analytics')) {
                assert.ok(
                  aliases('traffic-analytics').includes('traffic-analytics-ghost-example-com'),
                );
              }
            });
