...
import {run as runProcess} from '../manager/process.mjs';
import assert from 'node:assert/strict';
import fs from 'node:fs';
import path from 'node:path';
import {spawn} from 'node:child_process';
import {once} from 'node:events';
import {durableJSON, inventory, copyTree, verifyCheckpoint, checkSpace, retainCompleted} from '../manager/storage.mjs';
...
test('operation lock excludes another live process and recover cannot steal it', async () => {
    const dir = tempDir();
    const child = spawn(process.env.GD_TEST_BASH || 'bash', ['-c', `. scripts/lib/operation.sh; operation_acquire ${q(dir)}; echo ready; read -r done`], {stdio: ['pipe', 'pipe', 'pipe']});
    try {
    ...
    } finally { child.kill(); cleanup(dir); }
});
...
test('checkpoint validation detects corruption, missing files and forbidden links', () => {
    ...
    try {
        ...
        durableJSON(path.join(dir, 'manifest.json'), {format: 'ghost-docker-recovery', version: 1, files: inventory(payload)});
...
test('disk exhaustion leaves the previous durable journal readable', () => {
    ...
    } finally { fs.writeFileSync = original; cleanup(dir); }
});

test('retention preserves incomplete and corrupt checkpoints', () => {
    const dir = tempDir();
    try {
        const ids = ['2025-a-00000000-0000-0000-0000-000000000000', '2026-a-00000000-0000-0000-0000-000000000000'];
        for (const id of ids) {
            fs.mkdirSync(path.join(dir, id, 'payload'), {recursive: true});
            durableJSON(path.join(dir, id, 'manifest.json'), {format: 'ghost-docker-recovery', version: 1, files: []});
        }
...
test('SQL streaming propagates a producer or consumer failure despite partial output', async () => {
    ...
    try {
        ...
        await assert.rejects(runProcess('sh', ['-c', 'printf "partial SQL"; exit 17'], {output: dump}), /exit 17/);
        assert.equal(fs.readFileSync(dump, 'utf8'), 'partial SQL');
        await assert.rejects(runProcess('sh', ['-c', 'cat >/dev/null; exit 19'], {input: dump}), /exit 19/);
...
test('journal publication racing with lock acquisition still blocks mutation', () => {
    const dir = tempDir();
    try {
        const result = sh(`mkdir() { command mkdir "$@" && printf '{}' >${q(path.join(dir, '.ghost-operation.json'))}; }; operation_acquire ${q(dir)}`);
        assert.notEqual(result.status, 0);
        assert.equal(fs.existsSync(path.join(dir, '.ghost-operation-lock')), false);
    } finally { cleanup(dir); }
});
